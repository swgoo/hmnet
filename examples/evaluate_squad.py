...
class EvalArgs:
    ...
    na_prob_file: Optional[str] = None
    na_prob_thresh: float = 1.0
    out_image_dir: Optional[str] = None
    verbose: bool = False

    def to_list(self) -> List[str]:
        """Return a command-line-like list of arguments representing this config."""
        args: List[str] = [self.data_file, self.pred_file]
        if self.out_file:
            args.extend(["-o", self.out_file])
        if self.na_prob_file:
            args.extend(["-n", self.na_prob_file])
        # Only include threshold if it's been changed from the default
        if self.na_prob_thresh is not None and self.na_prob_thresh != 1.0:
            args.extend(["-t", str(self.na_prob_thresh)])
        if self.out_image_dir:
            args.extend(["-p", self.out_image_dir])
        if self.verbose:
            args.append("-v")
        return args
...
def parse_args(args: EvalArgs | None = None):
    ...
    if len(sys.argv) == 1:
        parser.print_help()
        sys.exit(1)
    return parser.parse_args(args.to_list() if args else None)
...
def main(args: EvalArgs | None = None):
